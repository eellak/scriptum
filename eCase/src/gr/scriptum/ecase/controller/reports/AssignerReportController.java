...
public class AssignerReportController extends BaseController {

	private static final long serialVersionUID = -4764822453190267965L;
	private static Log log = LogFactory.getLog(AssignerReportController.class);
	...
	private void createReport(int startIndex) throws InterruptedException {
		...
		Integer totalSize = dao.countTaskPerProjectForUser(getUserInSession());
		reportPgng.setTotalSize(totalSize);
		
		reportResults = dao.createTaskPerProjectForUser(getUserInSession(),
	...
	public void onClick$createBtn() throws Exception {
		createReport(0);
		log.info("PHRA TO REPORT");
		log.info("MEGETHOS APOTELESMATON:" + reportResults.size());
