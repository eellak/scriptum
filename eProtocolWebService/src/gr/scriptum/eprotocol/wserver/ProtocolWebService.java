...
public class ProtocolWebService implements EProtocolWebService{
	...
	@WebMethod
	public ProtocolReceipt receiveProtocol(  @WebParam(name = "message") ProtocolMessage message){
		...
		try{
			...
			//Create a RequestLogin in order to login into OpenKM and get security token
			...
			for (ProtocolDocument document : newIncoming
						.getProtocolDocuments()) {
			...
			if (newNodeResponse.isError()) {
				logger.warn("Could not create new OpenKM node " + newNodeResponse.toString());
				throw new Exception(newNodeResponse.toString());
			}else
				debug("New pending protocol created in OpenKm " + newIncoming.getId());
			...
			if (logoutResponse.isError()) {
				logger.warn("could not logout from OpenKM : " +  logoutResponse.toString());
	 ...
	 @WebMethod  
	public ProtocolInfo inquireProtocol( @WebParam(name = "query") ProtocolQuery query ){
		...
		try{
		   ...
		   //find the protocol
		   IncomingProtocolDAO incomingProtocolDAO = new IncomingProtocolDAO();
		   IncomingProtocol iprotocol = incomingProtocolDAO.findById( query.getProtocolId(), false);
		   if( iprotocol != null ){
	...
	//--------------------------------------------------
		...
		private String getInitParam(String paramName)   {
			...
			try{
			   ...
			   ServletContext serv = (ServletContext) mContext.get(MessageContext.SERVLET_CONTEXT);
	    ...
	    /* look for this mbean
		<mbean name="wsconfigurator.uit.gr:service=ProtocolWebServiceConfiguration"
		       code="gr.scriptum.eprotocol.wsconfigurator.ProtocolWebServiceConfiguration"
		       description="Used in order to configure from jmx console an active eProtocol Web Service.">    
		</mbean>
		*/
	...
	private IncomingProtocol incomingProtocolFromProtocolMessage( ProtocolMessage message ){
