...
public class ImapProtocolDispatcherImplTest {
	...
	public static void receiveMailFromIllumine() {
		...
		try {
			...
			IncomingProtocol[] receiveIncomingProtocols = disp
					.receiveIncomingProtocols();
			for (int i = 0; i < receiveIncomingProtocols.length; i++) {
				IncomingProtocol ip = receiveIncomingProtocols[i];
				System.out.println(ip.getSubject());
			}

			debug("FINISHED");
			debug("Received : " + receiveIncomingProtocols.length);
