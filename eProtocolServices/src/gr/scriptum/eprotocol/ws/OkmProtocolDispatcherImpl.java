...
import com.openkm.ws.endpoint.QueryParams;
import com.openkm.ws.endpoint.QueryParamsPropertiesEntry;
import com.openkm.ws.endpoint.QueryResult;
import com.openkm.ws.endpoint.RepositoryException;
import com.openkm.ws.endpoint.ResultSet;
...
public final class OkmProtocolDispatcherImpl  implements OkmProtocolDispatcher{
	  ...
	  String eCode = DISPACTER_ERROR.ECODE_NO_ERROR; 
	   ...
	   try{
		  if( deleteRequest.isDirectory() ){
			  ...
			  String path = deleteRequest.getUuid()!=null ? folderProxy.getPath(deleteRequest.getOkmToken(), deleteRequest.getUuid()) : deleteRequest.getPath();
			  folderProxy.delete(deleteRequest.getOkmToken(), path);
		  }else{
			  eCode = DISPACTER_ERROR.ECODE_DEL_FI;
			  OKMDocumentProxy documentProxy = (OKMDocumentProxy)getInstance(ProxyType.DOCUMENT);
			  String path = deleteRequest.getUuid()!=null ? documentProxy.getPath(deleteRequest.getOkmToken(), deleteRequest.getUuid()) : deleteRequest.getPath();
			  documentProxy.delete(deleteRequest.getOkmToken(), path);
		  }
	   }catch(Throwable  e){
   ...
   @Override
   public ResponseIsNodeValid isNodeValid(RequestIsNodeValid isNodeValidRequest) {
	   ResponseIsNodeValid isNodeValidResponse = new ResponseIsNodeValid(isNodeValidRequest);
	   
		try{
			OKMFolderProxy folderProxy = (OKMFolderProxy)getInstance(ProxyType.FOLDER);		
			boolean valid = folderProxy.isValid(isNodeValidRequest.getOkmToken(), 
					isNodeValidRequest.getFolderPath() );
			isNodeValidResponse.setValid(valid);
		
		}catch(Throwable  e ){
			if(e instanceof PathNotFoundException) {
				isNodeValidResponse.setValid(Boolean.FALSE);
				return isNodeValidResponse;
			}else {
				isNodeValidResponse = (ResponseIsNodeValid) createErrorResponse( isNodeValidResponse,  e, DISPACTER_ERROR.ECODE_ND_NOT_VALID);
			}
		}
    ...
    */
	...
	public ResponseSearch searchByContent( RequestSearch  req){
		...
		try{
			...
			params.setDomain(1);
			params.setId(0);
			params.setContent(req.getContent());
			params.setProperties(new QueryParamsPropertiesEntry[] {});
			
			
			ResultSet paginated = searchProxy.findPaginated(req.getOkmToken(), params , req.getOffset(), req.getLimit());
			resp.setTotal(paginated.getTotal());
			
//			QueryResult[] results = searchProxy.findByContent( req.getOkmToken(), req.getContent());
			QueryResult[] results = paginated.getResults();
			if( results != null){
	...
	public ResponseSendDocument sendDocument(RequestSendDocument req){
		...
		try{	
			OKMDocumentProxy documentProxy = (OKMDocumentProxy)getInstance(ProxyType.DOCUMENT);	
			String path = documentProxy.getPath(req.getOkmToken(), req.getDocumentPath());
			if( documentProxy.isValid(req.getOkmToken(), path) ){
				openKmDoc  = documentProxy.getProperties(req.getOkmToken(), path);
				content = documentProxy.getContent(req.getOkmToken(), path, true);
   ...
   //-----------
     ...
     * qualified with a uuid . This is held in okmDoc.  
